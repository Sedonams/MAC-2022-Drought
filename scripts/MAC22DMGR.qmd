...
```{r,warning=FALSE,message=FALSE}
MAC22_MLM_shoot_wt <- shoot_wt ~ treatment * genotype + (1|num_of_plants) + (1|rep)
```
...
boxplot(dry_root_wt ~ age:treatment, data = ds)
...
leveneTest(total_biomass ~ age, data = ds)
...
############################
#trying somthing
MAC22_MLM_total_biomass <- lmer(total_biomass ~ treatment * genotype + age +(1|num_of_plants) + (1|rep), data = ds)
...
m <- lm(data=ds, total_biomass ~ treatment + genotype + num_of_plants + age )
vif(m)

plot(data=ds, age~total_biomass)
M.loess <- lm(age~total_biomass, data = ds)
Fit <- fitted(M.loess)
head(Fit)
...
model4 <- lm(total_biomass ~ age * treatment, data = ds)
summary(model4)
anova(model4)
# Type III ANOVA to assess main effects while accounting for interaction
Anova(model4, type = 3)
plot_model(model4, type = "pred", terms = c("age", "treatment"))

library(emmeans)
# Estimate slope for each sex group and test if slopes differ
slope_tests <- emtrends(model4, ~ treatment, var = "age")

# Compare slopes between sexes (test interaction)
pairs(slope_tests)
test(slope_tests)

model5 <- lm(total_biomass ~ treatment + genotype + num_of_plants + age, data = ds)
summary(model5)
Anova(model5, type=2)
...
ds1 <- ds %>% mutate(across(c(age, num_of_plants), list(s = scale)))
model7 <- lm(total_biomass ~ age_s + num_of_plants_s, data = ds1)
summary(model7)
car::vif(model7)
...
ds$Z_age <- scale(ds$age)

hist(ds$Z_age, xlab = "Z_age", main = "")

## Create a linear model without random effects
lm.test <- lm(Z_tot ~ Z_age, data = ds)
summary(lm.test)

## plot the data with ggplot
prelim_plot <- ggplot(ds, aes(x = Z_age, y = Z_tot, color = treatment)) +
  geom_point() + theme_bw()+
  geom_smooth(method = "lm", se=FALSE)+
    facet_wrap(~genotype)
prelim_plot

prelim_plot <- ggplot(ds, aes(x = age, y = total_biomass, color = treatment)) +
  geom_point() + theme_bw()+
  geom_smooth(method = "lm", se=FALSE)
prelim_plot
...
library(nlme)
M1.lme= lme(Z_tot ~ Z_age, random=~1|treatment, 
     data = ds, method="REML")

# Now add random effects
M1.lmer <- lmer(Z_tot ~ Z_age + (1 | genotype), data = ds)
...
M1.lme= lme(Z_age ~ Z_tot, random=~1|treatment, 
     data = ds, method="ML")

M2.lme= lme(Z_age ~ 1, random=~1|treatment, 
     data = ds, method="ML")

anova(M1.lme, M2.lme)


M1.lme= lme(Z_age ~ Z_tot, random=~1|genotype, 
     data = ds, method="REML")
summary(M1.lme)
...
ggplot(pred.mm) + 
   ...
   geom_point(data = ds,                      # adding the raw data (scaled values)
              aes(x = Z_tot, y = Z_age, colour = treatment)) + 
   labs(x = "tot wt", y = "age", 
        title = "All data") + 
   fig
...
m3 <- lmer((total_biomass) ~ (treatment) + (genotype) + (1 | age) + (1| rep) , data = ds)
summary(m3)


Mbea <- lmer((total_biomass) ~ (treatment) + (genotype) +  (1| rep) + (1 | age) + (1 | purpleness), data = ds)
summary(Mbea) 
...
# Option A: treat num_of_plants and age as random effects
...
ggplot(ds, aes(x = age, y = dry_root_wt)) +
  ...
  ggplot(ds, aes(x = age, y = dry_root_wt, color = treatment)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE) +
  theme_minimal()
...
GGally::ggpairs(ds[, c("total_biomass", "dry_root_wt", "age", "num_of_plants", "rep")])
#col test
...
cor(ds[, c("total_biomass", "dry_root_wt", "age", "num_of_plants", "rep")], use = "pairwise.complete.obs")    
pairs(ds[, c("total_biomass", "dry_root_wt", "age", "num_of_plants", "rep")], upper.panel = NULL)   
library(car)
vif(m2)

lm1 <- lm(dry_root_wt ~ treatment + genotype + age, data = ds)
summary(lm1)
...
gls1 <- gls(dry_root_wt ~ treatment + genotype + age,
            data = ds,
            method = "REML",
            na.action = na.exclude)

# 2️⃣ Fit LME model with replicate as random effect
lme1 <- lme(dry_root_wt ~ treatment + genotype + age,
            random = ~ 1 | rep,
            data = ds,
            method = "REML",
            na.action = na.exclude)
...
lme_final <- update(lme1, fixed = dry_root_wt ~ treatment + genotype + age,
                    random = ~ 1 | rep,
                    method = "REML")
summary(lme_final)
...
lmer_final <- lmer(dry_root_wt ~ genotype + age + (1 | rep),
                   data = ds,
                   REML = TRUE)
...
pred <- ggpredict(lmer_final, terms = "age")
...
for (fold in 1:k) {
  ...
  model <- lmer(dry_root_wt ~ genotype + age + (1|rep),
                data = train_data)
