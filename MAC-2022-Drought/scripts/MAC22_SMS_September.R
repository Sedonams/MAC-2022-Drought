...
ds <- ds %>%
  mutate(
    arb_ves_and_arb = rowSums(dplyr::select(., arb, ves_and_arb), na.rm = TRUE),
    vesicle_or_spore_ves_and_arb = rowSums(dplyr::select(., vesicle_or_spore, ves_and_arb), na.rm = TRUE),
    am_hyphae_dse_and_am = rowSums(dplyr::select(., am_hyphae, dse_and_am), na.rm = TRUE),
    dse_dse_and_am = rowSums(dplyr::select(., dse, dse_and_am), na.rm = TRUE)
  )
...
dotchart(ds$shoot_wt, group = factor(ds$treatment))
boxplot(shoot_wt ~ rep:treatment, data = ds, na.action = na.omit)
boxplot(main_shoot_wtkg ~ genotype:treatment, data = ds, na.action = na.omit)
boxplot(shoot_wt ~ genotype:treatment, data = ds, na.action = na.omit)

# Linear model for total biomass by genotype and treatment

m <- lm(shoot_wt ~ genotype + treatment, data = ds, na.action = na.omit)
...
#n <- lm(ds$shoot_wt ~ ds$genotype + ds$treatment + (1 | rep), na.action = na.omit)
#n
#str(n)
...
leveneTest(shoot_wt ~ treatment, data = ds, na.action = na.omit)
...
if(length(influential) > 0) {
  ...
  print(ds[influential, c("genotype", "treatment", "shoot_wt")])
}
...
# 2️⃣ Fit LME model with replicate
lme1 <- lme(dry_root_wt ~ treatment + genotype + sprout_or_transplant_to_harvest + rep,
            data = ds,
            method = "REML",
            na.action = na.exclude)
...
lmer_final <- lmer(dry_root_wt ~ treatment + genotype + sprout_or_transplant_to_harvest + (1 | rep),
                   data = ds,
                   REML = TRUE)
...
ds_clean <- ds %>%
  dplyr::select(dry_root_wt, treatment, genotype, sprout_or_transplant_to_harvest, rep) %>%
  na.omit()

# Refit model on clean data (optional but safest)
lmer_final <- lmer(
  dry_root_wt ~ treatment + genotype + sprout_or_transplant_to_harvest + rep,
  data = ds_clean,
  REML = TRUE
)

# Get marginal effects for harvest age
pred <- ggpredict(
  lmer_final,
  terms = "sprout_or_transplant_to_harvest [all]"
)

# Plot
plot(pred) +
  labs(
    title = "Effect of Harvest Age on Dry Root Weight",
    x = "Days from Sprout/Transplant to Harvest",
    y = "Predicted Dry Root Weight (g)"
  ) +
...
ds_clean <- na.omit(ds[, c("dry_root_wt", "treatment", "genotype", "sprout_or_transplant_to_harvest", "rep")])

lmer_final_ml <- lmer(
  dry_root_wt ~ treatment * genotype + sprout_or_transplant_to_harvest + (1 | rep),
  data = ds_clean,
  REML = FALSE
)
...
# Ensure factors are consistent across folds
ds_clean$treatment <- factor(ds_clean$treatment)
ds_clean$rep <- factor(ds_clean$rep)

# Number of folds
k <- 5

# Assign folds by rep
groups <- unique(ds_clean$rep)
...
for (fold in 1:k) {
  ...
  train_data <- ds_clean[!ds_clean$rep %in% test_groups, ]
  test_data  <- ds_clean[ds_clean$rep %in% test_groups, ]
  
  # Re-level factors in test to match training
  test_data$treatment <- factor(test_data$treatment, levels = levels(train_data$treatment))
  test_data$rep <- factor(test_data$rep, 
  levels = levels(train_data$rep))
  
  # Fit mixed model on training data
  model <- try(lmer(dry_root_wt ~ treatment + genotype + sprout_or_transplant_to_harvest,
                    data = train_data),
               silent = TRUE)
  
  # Skip fold if model failed
  if (inherits(model, "try-error")) {
    cv_results <- rbind(cv_results, data.frame(Fold = fold, R2 = NA, RMSE = NA))
    next
  }
  
  # Predict on test data (allow.new.levels=TRUE handles unseen reps)
...
cv_summary <- data.frame(
  ...
  SD_RMSE   = sd(cv_results$RMSE, na.rm = TRUE)
)
