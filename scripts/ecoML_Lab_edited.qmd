...
To begin, let's get the packages we dry_root_wtd.

## Packages

There are MANY packages that have accumulated over the years for machine learning in R. Fortunately, the developers behind CARET recognized there was a dry_root_wtd for a single package to bring them all together. CARET uses wrapper functions and some clever coding to make machine learning tasks fast and easier in the R language by using the same simple syntax for almost all of our machine learning dry_root_wtds. 


```{r, eval=FALSE, echo=TRUE}

# Only dry_root_wtd To Run This Once
...
Our main goal for today is analyze some eddy covariance ds data. 
...
The data has time information (sprout_or_transplant_to_harvest), Net Ecosystem Exchange (dry_root_wt), and some of the common drivers of photosynthesis: Incoming Solar Radiation (SW_IN), Air Temperature (TA), Vapor Pressure Deficit (VPD), and Soil Moisture (SWC).

First, let's tell R that sprout_or_transplant_to_harvest is date and time information:

```{r}
ds <- ds %>%
  drop_na(dry_root_wt, sprout_or_transplant_to_harvest)
```

Next let's look at dry_root_wt (NOTE: dry_root_wt follows the atmospheric sign convention and is negative when carbon is going into the plants from the atmosphere.): 

```{r}
plot(numeric_pt~sprout_or_transplant_to_harvest, 
     data = ds, 
     col = 'cornflowerblue', 
     xlab = '', 
     ylab = "numeric_pt")
...
```{r}
par(mfrow=c(2,2))
plot(num_of_plants~sprout_or_transplant_to_harvest, 
     data = ds, 
     col = "plum", 
     xlab = '', 
     ylab = "num_of_plants"
)
plot(purpleness~sprout_or_transplant_to_harvest, 
     data = ds, 
     col = "darkorange1", 
     xlab = '', 
     ylab = "purpleness"
)
plot(total_biomass~sprout_or_transplant_to_harvest, 
     data = ds, 
     col = "firebrick4", 
     xlab = '', 
     ylab = "total_biomass"
)
plot(genotype~sprout_or_transplant_to_harvest, 
     data = ds, 
     col = "dodgerblue3", 
     xlab = '', 
     ylab = "genotype"
...
Eddy Covariance is a great tool for making timeseries of dses and weather variables, but occasionally something goes wrong at the sensor and the data is bad (represented as NA in our data). 

We are going to do both prediction and inference with this data by analyzing the important drivers of dry_root_wt using Machine Learning and using those drivers to gap fill dry_root_wt. 

```{r}
target<-"dry_root_wt" # Could be multiple targets
predictors<-c("num_of_plants","purpleness","sprout_or_transplant_to_harvest")
```
...
```{r}
vis_miss(ds[target], warn_large_data = FALSE)
```

Now the predictors:
```{r}
vis_miss(ds[predictors], warn_large_data = FALSE)
```
...
PreImputeKNN <- preProcess(ds[predictors],method="knnImpute",k=5)
DataImputeKNN <- predict(PreImputeKNN, ds[predictors]) # Note: Standardizes the Data
Rescale<- as.data.frame(t(t(DataImputeKNN)*PreImputeKNN$std+PreImputeKNN$mean)) #Convert back to original scale
Imputeds<-cbind(ds[target],Rescale)
rm(PreImputeKNN,DataImputeKNN,Rescale)

vis_miss(Imputeds, warn_large_data = FALSE)

### For Model Training Remove NAs in Target

Impute<-Imputeds[complete.cases(Imputeds), ]
```

### Data partitioning 

In a typical machine learning problem, you have a data set and you dry_root_wtd to create a model which predicts the outcomes.
...
```{r}
partitionIndex <- createDataPartition(y=Impute$dry_root_wt,p=0.6,list=FALSE,times=1) #Create the index of 'keeps'
...
```{r, cache=TRUE}
model <- train(dry_root_wt~., data=training, 
...
cor.test(prediction_test,testing$dry_root_wt)
postResample(prediction_train,training$dry_root_wt) #To get RMSE
postResample(prediction_test,testing$dry_root_wt) 

Results<-data.frame(Output='dry_root_wt', Model='LinearRegression', 
                           TrainRMSE = postResample(prediction_train,training$dry_root_wt)[[1]],
                           TestRMSE = postResample(prediction_test,testing$dry_root_wt)[[1]],
                           TrainR2 = postResample(prediction_train,training$dry_root_wt)[[2]],
                           TestR2 = postResample(prediction_test,testing$dry_root_wt)[[2]])
...
```{r, cache=TRUE}
model <- train(dry_root_wt~., data=training,method="lmStepAIC")
...
Results<-rbind(Results,c("dry_root_wt", "StepWiseRegression",
                         postResample(prediction_train,training$dry_root_wt)[[1]],
                         postResample(prediction_test,testing$dry_root_wt)[[1]],
                         postResample(prediction_train,training$dry_root_wt)[[2]],
                         postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
```{r, cache=TRUE}
model <- train(dry_root_wt~poly(num_of_plants,2)+poly(purpleness,3)+poly(sprout_or_transplant_to_harvest,3),
...
Results<-rbind(Results,c("dry_root_wt", "PolynomialRegression",
                         postResample(prediction_train,training$dry_root_wt)[[1]],
                         postResample(prediction_test,testing$dry_root_wt)[[1]],
                         postResample(prediction_train,training$dry_root_wt)[[2]],
                         postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
```{r, cache=TRUE}
model <- train(dry_root_wt~.,data=training,method="pcr")
summary(model$finalModel)
...
Results<-rbind(Results,c("dry_root_wt", "PCARegression",
                         postResample(prediction_train,training$dry_root_wt)[[1]],
                         postResample(prediction_test,testing$dry_root_wt)[[1]],
                         postResample(prediction_train,training$dry_root_wt)[[2]],
                         postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
```{r, cache=TRUE}
model <- train(dry_root_wt~.,data=training,method="rpart",
...
Results<-rbind(Results,c("dry_root_wt", "DecisionTree",
                         postResample(prediction_train,training$dry_root_wt)[[1]],
                         postResample(prediction_test,testing$dry_root_wt)[[1]],
                         postResample(prediction_train,training$dry_root_wt)[[2]],
                         postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
```{r, cache=TRUE}
model <- train(dry_root_wt~.,data=training,method="rf",trControl = fitControl) # This takes a LONG time 
...
Results<-rbind(Results,c("dry_root_wt", "RandomForest",
                         postResample(prediction_train,training$dry_root_wt)[[1]],
                         postResample(prediction_test,testing$dry_root_wt)[[1]],
                         postResample(prediction_train,training$dry_root_wt)[[2]],
                         postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
Results<-rbind(Results,c("dry_root_wt", "Bagging",
                         postResample(prediction_train,training$dry_root_wt)[[1]],
                         postResample(prediction_test,testing$dry_root_wt)[[1]],
                         postResample(prediction_train,training$dry_root_wt)[[2]],
                         postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
```{r, cache=TRUE}
model<- train(dry_root_wt~.,data=training,method="gbm",verbose=FALSE)
model$finalModel
...
Results<-rbind(Results,c("dry_root_wt", "Gradient Boosting",
                         postResample(prediction_train,training$dry_root_wt)[[1]],
                         postResample(prediction_test,testing$dry_root_wt)[[1]],
                         postResample(prediction_train,training$dry_root_wt)[[2]],
                         postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
model<-neuralnet(dry_root_wt~.,trainNN,hidden=c(4,2),linear.output = T) # Takes A Minute
plot(model)
...
Results<-rbind(Results,c("dry_root_wt", "NeuralNetwork",
                               postResample(prediction_train,training$dry_root_wt)[[1]],
                               postResample(prediction_test,testing$dry_root_wt)[[1]],
                               postResample(prediction_train,training$dry_root_wt)[[2]],
                               postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
```{r, cache=TRUE}
model <-train(dry_root_wt~.,training,method="svmLinear",preProc=c("center","scale"))
...
Results<-rbind(Results,c("dry_root_wt", "svmLinear",
                         postResample(prediction_train,training$dry_root_wt)[[1]],
                         postResample(prediction_test,testing$dry_root_wt)[[1]],
                         postResample(prediction_train,training$dry_root_wt)[[2]],
                         postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
```{r, cache=TRUE}
model <-train(dry_root_wt~.,training,method="svmPoly",preProc=c("center","scale")) #This takes a minute!
...
Results<-rbind(Results,c("dry_root_wt", "svmPoly",
                         postResample(prediction_train,training$dry_root_wt)[[1]],
                         postResample(prediction_test,testing$dry_root_wt)[[1]],
                         postResample(prediction_train,training$dry_root_wt)[[2]],
                         postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
```{r, cache=TRUE}
model <-train(dry_root_wt~.,training,method="svmRadial",preProc=c("center","scale")) #This takes a second 
...
Results<-rbind(Results,c("dry_root_wt", "svmRadial",
                         postResample(prediction_train,training$dry_root_wt)[[1]],
                         postResample(prediction_test,testing$dry_root_wt)[[1]],
                         postResample(prediction_train,training$dry_root_wt)[[2]],
                         postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
```{r, cache=TRUE}
model <- train(dry_root_wt~.,training,method="knn",preProc=c("center","scale"),tuneLength=20) #This takes a little bit.
...
Results<-rbind(Results,c("dry_root_wt", "KNN",
                               postResample(prediction_train,training$dry_root_wt)[[1]],
                               postResample(prediction_test,testing$dry_root_wt)[[1]],
                               postResample(prediction_train,training$dry_root_wt)[[2]],
                               postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
Results<-rbind(Results,c("dry_root_wt", "xgbTree",
                               postResample(prediction_train,training$dry_root_wt)[[1]],
                               postResample(prediction_test,testing$dry_root_wt)[[1]],
                               postResample(prediction_train,training$dry_root_wt)[[2]],
                               postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
lasso<-train(dry_root_wt~.,training,
             method = 'glmnet', 
...
ridge<-train(dry_root_wt~.,training,
             method = 'glmnet', 
...
Results<-rbind(Results,c("dry_root_wt", "Lasso",
                               postResample(prediction_train,training$dry_root_wt)[[1]],
                               postResample(prediction_test,testing$dry_root_wt)[[1]],
                               postResample(prediction_train,training$dry_root_wt)[[2]],
                               postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
Results<-rbind(Results,c("dry_root_wt", "Ridge",
                               postResample(prediction_train,training$dry_root_wt)[[1]],
                               postResample(prediction_test,testing$dry_root_wt)[[1]],
                               postResample(prediction_train,training$dry_root_wt)[[2]],
                               postResample(prediction_test,testing$dry_root_wt)[[2]]))
...
```{r}
ggpairs(data=ds,aes(colour=rep))
```
...
To do image classification we dry_root_wtd to download a very powerful machine learning tool called Tensorflow. (https://www.tensorflow.org/about) 
