...
```{r,warning=FALSE,message=FALSE}
m <- lmer(
  tot_shoot_wtg ~ amf_rlc * treatment + genotype + (1|rep),
  data = ds)
...
geno_summary <- ds %>%
  group_by(genotype, treatment) %>%
  summarise(
    mean_shoot = mean(tot_shoot_wtg, na.rm = TRUE),
    mean_vesicle = mean(vesicle_rlc, na.rm = TRUE),
    .groups = "drop"
  )
...
ggplot() +
  geom_point(data = geno_summary,
             aes(mean_vesicle, mean_shoot, color = treatment),
             alpha = 0.5) +
...
geno_summary <- ds %>%
  ...
  summarise(
    mean_shoot = mean(tot_shoot_wtg, na.rm = TRUE),
    mean_arb = mean(arb_rlc, na.rm = TRUE),
    .groups = "drop"
  )
...
ggplot() +
  geom_point(data = geno_summary,
             aes(mean_arb, mean_shoot, color = treatment),
             alpha = 0.5) +
...
drought_perf <- geno_summary %>%
  ...
  arrange(desc(mean_shoot))

drought_perf %>%
  select(genotype, mean_shoot, mean_rlc)


lm(mean_shoot ~ mean_rlc, data = filter(geno_summary, treatment == "Droughted"))
summary(lm(mean_shoot ~ mean_rlc,
...
geno_wide <- geno_summary %>%
  tidyr::pivot_wider(
    ...
    values_from = c(mean_shoot, mean_rlc)
  )

lm(drought_advantage ~ mean_rlc, data = geno_wide)
summary(lm(drought_advantage ~ mean_rlc, data = geno_wide))

ggplot(geno_summary, aes(mean_rlc, mean_shoot, color = treatment)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(~genotype) +
  theme_classic()
...
ggplot(geno_summary,
       aes(treatment, mean_shoot, group = genotype)) +
  geom_line(alpha = 0.4) +
  geom_point() +
  theme_classic()
...
ggplot(geno_slopes, aes(x = reorder(genotype, slope), y = slope)) +
  ...
  labs(
    x = "Genotype",
    y = "Colonization → Growth Slope",
    title = "Genotype Response to AMF Colonization"
  )

geno_slopes_drought <- ds %>%
  filter(treatment == "Droughted") %>%
  group_by(genotype) %>%
  do({
    model <- lm(tot_shoot_wtg ~ amf_rlc, data = .)
...
ggplot(geno_slopes_drought, aes(x = reorder(genotype, slope), y = slope)) +
  geom_col(fill = "steelblue") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  coord_flip() +
  theme_classic() +
  labs(
    x = "Genotype",
    y = "Colonization → Growth Slope",
    title = "Genotype Response to AMF Colonization in Drought Only"
  )
...
numeric_vars <- ds %>%
  dplyr::select(dry_root_wt, shoot_wt, tot_shoot_wtg, 
