...
boxplot(amf_rlc ~ genotype:treatment, data = ds)
ggplot(ds, aes(x = amf_rlc) ) +
  geom_histogram(bins = 30, fill = "steelblue", color = "black") +
  theme_bw() +
  labs(x = "AMF Root Length Colonized (%)", y = "Frequency",
...
geno_summary <- ds %>%
  ...
  summarise(
    across(
      c(tot_shoot_wtg, amf_rlc, arb_rlc, vesicle_rlc, olpidium_rlc,
        am_hyphae_rlc, dse_rlc, lse_rlc, coil_rlc, fine_endo_rlc, non_am_rlc),
      ~ mean(.x, na.rm = TRUE),
      .names = "mean_{.col}"
    ),
    .groups = "drop"
...
geno_long <- geno_summary %>%
  pivot_longer(
    ...
    names_to = "fungal_var",
    values_to = "colonization"
  )

ggplot(geno_long,
       aes(colonization, mean_tot_shoot_wtg, color = treatment)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 1.1) +
  facet_wrap(~ fungal_var, scales = "free_x") +
...
ggplot(geno_summary, aes(mean_amf_rlc, mean_tot_shoot_wtg, color = treatment)) +
  ...
  theme_classic(base_size = 14) +
  labs(
    x = "Mean AMF Colonization (%)",
    y = "Mean Shoot Biomass (g)",
    color = "Treatment"
  )

ggplot(geno_summary, aes(mean_arb_rlc, mean_tot_shoot_wtg, color = treatment)) +
  geom_point(size = 4) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 1.2) +
  theme_classic(base_size = 14) +
  labs(
    x = "Mean Arbuscule Colonization (%)",
    y = "Mean Shoot Biomass (g)",
    color = "Treatment"
  )

ggplot(geno_summary, aes(mean_vesicle_rlc, mean_tot_shoot_wtg, color = treatment)) +
  geom_point(size = 4) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 1.2) +
  theme_classic(base_size = 14) +
  labs(
    x = "Mean Vesicle Colonization (%)",
    y = "Mean Shoot Biomass (g)",
    color = "Treatment"
  )
#
...
# Model n: GLS (fixed effects only, no random effects)
# If you want to account for rep as a correlation structure instead:
n <- gls(tot_shoot_wtg ~ amf_rlc * treatment + genotype,
  correlation = corCompSymm(form = ~ 1 | rep),  # equivalent to random intercept
  data = ds)

# Model o: three-way interaction, random intercept for rep
o <- lmer(tot_shoot_wtg ~ amf_rlc * treatment * genotype + (1|rep),
  data = ds)

# AIC doesn't directly compare lmer and gls objects well — use same framework
# Option 1: keep all as lmer
AIC(m,o)  # compare just the lmer models

# Option 2: refit m with gls for a fair comparison
m_gls <- gls(tot_shoot_wtg ~ amf_rlc * treatment + genotype,
  correlation = corCompSymm(form = ~ 1 | rep),
  data = ds)

View(AIC(m_gls, n, m,))  # now comparable


DHARMa::simulateResiduals(fittedModel = m, plot = TRUE)

pred <- ggpredict(m, terms = c("amf_rlc", "treatment"))

ggplot() +
  geom_point(data = ds,
             aes(amf_rlc, tot_shoot_wtg, color = treatment),
             alpha = 0.5) +
  ...
  labs(
    x = "AMF Colonization (%)",
    y = "Total Shoot Biomass (g)",
    color = "Treatment",
    fill = "Treatment")


m <- lmer(
  amf_rlc ~ treatment + genotype + (1|rep),
  data = ds)
...
DHARMa::simulateResiduals(fittedModel = n, plot = TRUE)
AIC(m,n,o)
...
m <- lmer(
  tot_shoot_wtg ~ vesicle_rlc * treatment + genotype + (1|rep),
  data = ds)

pred <- ggpredict(m, terms = c("vesicle_rlc", "treatment"))

ggplot() +
  geom_point(data = geno_summary,
             aes(mean_vesicle_rlc, mean_tot_shoot_wtg, color = treatment),
             alpha = 0.5) +
  ...
  theme_classic(base_size = 14) +
  labs(
    x = "Mean Vesicle Root Length Colonized (%)",
    y = "Mean Total Shoot Biomass (g)",
    color = "Treatment",
    fill = "Treatment")

m <- lmer(
  tot_shoot_wtg ~ arb_rlc * treatment + genotype + (1|rep),
  data = ds)

pred <- ggpredict(m, terms = c("arb_rlc", "treatment"))

ggplot() +
  geom_point(data = geno_summary,
             aes(mean_arb_rlc, mean_tot_shoot_wtg, color = treatment),
             alpha = 0.5) +
  geom_ribbon(data = pred,
              aes(x = x, ymin = conf.low, ymax = conf.high, fill = group),
              alpha = 0.25) +
  ...
  theme_classic(base_size = 14) +
  labs(
    x = "Mean Arbuscule Root Length Colonized (%)",
    y = "Mean Total Shoot Biomass (g)",
    color = "Treatment",
    fill = "Treatment")
...
summary(lm(mean_tot_shoot_wtg ~ mean_rlc,
           data = filter(geno_summary, treatment == "Droughted")))

model <- lmer(
  tot_shoot_wtg ~ amf_rlc * treatment + (1 | genotype),
  data = ds
)
summary(model)

ggplot(ds, aes(amf_rlc, tot_shoot_wtg, color = treatment)) +
...
ggplot(geno_summary,
       aes(treatment, mean_tot_shoot_wtg, group = genotype)) +
  geom_line(alpha = 0.4) +
  geom_point() +
  theme_classic()


# Combine Watered and Droughted if you want side-by-side panels
geno_slopes <- ds %>%
  group_by(treatment, genotype) %>%
  do({
    model <- lm(tot_shoot_wtg ~ amf_rlc, data = .)
    s <- summary(model)$coefficients
    data.frame(
      slope = s["amf_rlc", "Estimate"],
      se = s["amf_rlc", "Std. Error"]
    )
  }) %>%
  mutate(
    lower = slope - 1.96 * se,
...
# Reorder genotypes within each treatment
geno_slopes <- geno_slopes %>%
  group_by(treatment) %>%
  arrange(slope) %>%
...
# Plot
ggplot(geno_slopes, aes(x = genotype, y = slope)) +
  geom_errorbar(aes(ymin = lower, ymax = upper),
                width = 0.2, color = "gray70") +
  geom_point(aes(color = ifelse(significant, treatment, "non-significant"),
                 size = significant)) +
  ...
  scale_color_manual(values = c(
    ...
    "Watered" = "#1b9e77",  # replace with your treatment colors
    "Droughted" = "#d95f02"
  )) +
...
theme(
  ...
  strip.background = element_blank(),       # <-- removes boxes
  strip.text = element_text(face = "bold"), # keeps bold header text
  ...
  labs(
    x = "Genotype",
    y = "Slope of Biomass vs AMF Colonization",
...
# another model 
n <- lmer(amf_rlc ~ genotype * treatment, data = ds)
n
str(n)
...
leveneTest(tot_shoot_wtg ~ sprout_or_transplant_to_harvest, data = ds)
...
if(length(influential) > 0) {
  ...
  print(ds[influential, c("genotype", "treatment", "rep", "tot_shoot_wtg")])
}
...
############################
#trying somthing
MAC22_MLM_tot_shoot_wtg <- lmer(tot_shoot_wtg ~ treatment * genotype + sprout_or_transplant_to_harvest +(1|num_of_plants) + (1|rep), data = ds)
plot(residuals(MAC22_MLM_tot_shoot_wtg)) # Example: plot residuals
plot(MAC22_MLM_tot_shoot_wtg)
#
...
ds_bartlett <- ds %>% 
  ...
  filter(n() >= 2) %>% 
  ungroup()
m <- bartlett.test(tot_shoot_wtg ~ interaction(treatment, genotype), data = ds_bartlett)
# Bartlett test does not return residuals or fitted values, so plotting them is not applicable.
print(m)
####################################

hist (ds$amf_rlc)
hist (ds$tot_shoot_wtg)
hist (n$residuals)
...
table(ds$tot_shoot_wtg == 0)

plot(ds)
library(car)

# Check for multicollinearity using Variance Inflation Factor (VIF)
m <- lm(data=ds, tot_shoot_wtg ~ treatment + genotype + num_of_plants + sprout_or_transplant_to_harvest )
vif(m)

plot(data=ds, sprout_or_transplant_to_harvest~tot_shoot_wtg)
M.loess <- lm(sprout_or_transplant_to_harvest~tot_shoot_wtg, data = ds)
Fit <- fitted(M.loess)
head(Fit)
lines(x = ds$tot_shoot_wtg, y = Fit)
...
coplot(tot_shoot_wtg ~ num_of_plants | treatment,
       data = ds,
       panel = function(x, y, ...) {
...
ggplot(ds, aes(x = treatment, y = tot_shoot_wtg, fill = treatment)) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Accent") +
  theme_bw()

model2 <- lm(tot_shoot_wtg ~ treatment, data = ds)
summary(model2)
tab_model(model2)
...
ggplot(ds, aes(x = position, y = tot_shoot_wtg, fill = position)) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Accent") +
  theme_bw()

ggplot(ds, aes(x = rep, y = amf_rlc, fill = rep)) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Accent") +
  theme_bw()

model3 <- lm(tot_shoot_wtg ~ rep, data = ds)
summary(model3)
...
model4 <- lm(tot_shoot_wtg ~ sprout_or_transplant_to_harvest * treatment, data = ds)
summary(model4)
anova(model4)
...
model5 <- lm(tot_shoot_wtg ~ treatment + genotype + num_of_plants + sprout_or_transplant_to_harvest, data = ds)
summary(model5)
Anova(model5, type=2)
...
model7 <- lm(tot_shoot_wtg ~ sprout_or_transplant_to_harvest_s + num_of_plants_s, data = ds1)
summary(model7)
car::vif(model7)
...
numeric_vars <- ds %>%
  dplyr::select(dry_root_wt, tot_shoot_wtg, 
...
# Prepare data
  shap_data <- ds %>%
    select(
    tot_shoot_wtg,  # response
    amf_rlc,
  ) %>%
  na.omit()

X <- shap_data[, setdiff(names(shap_data), "tot_shoot_wtg")]
y <- shap_data$tot_shoot_wtg
...
colonization_vars <- c("amf_rlc", "am_hyphae_rlc", "arb_rlc", 
                       "dse_rlc", "vesicle_rlc", 
...
main = "")
hist(ds$tot_shoot_wtg, xlab = "tot_shoot_wtg", main = "")

# Standardized tot_shoot_wtg, "by hand"
ds$Z_tot <- (ds$tot_shoot_wtg - mean(ds$tot_shoot_wtg)) / 
                      sd(ds$tot_shoot_wtg)
...
prelim_plot <- ggplot(ds, aes(x = sprout_or_transplant_to_harvest, y = tot_shoot_wtg, color = treatment)) +
  geom_point() + theme_bw()+
  geom_smooth(method = "lm", se=FALSE)
prelim_plot

prelim_plot <- ggplot(ds, aes(x = num_of_plants, y = tot_shoot_wtg, color = treatment)) +
  geom_point() + theme_bw()+
  geom_smooth(method = "lm", se=FALSE)+ 
  facet_wrap(~genotype)
prelim_plot
...
m3 <- lmer((tot_shoot_wtg) ~ (treatment) + (genotype) + (1 | sprout_or_transplant_to_harvest) + (1| rep) , data = ds)
summary(m3)


Mbea <- lmer((tot_shoot_wtg) ~ (treatment) + (genotype) +  (1| rep) + (1 | sprout_or_transplant_to_harvest) + (1 | purpleness), data = ds)
summary(Mbea) 

names(ds)

p <- ggplot(ds, aes(x = tot_shoot_wtg, y = numeric_pt)) +
...
m1 <- lmer(tot_shoot_wtg ~ treatment * genotype +
             (1 | rep) +
             (1 | num_of_plants) +
...
GGally::ggpairs(ds[, c("tot_shoot_wtg", "dry_root_wt", "sprout_or_transplant_to_harvest", "num_of_plants", "rep")])
#col test
...
cor(ds[, c("tot_shoot_wtg", "dry_root_wt", "sprout_or_transplant_to_harvest", "num_of_plants", "rep")], use = "pairwise.complete.obs")    
pairs(ds[, c("tot_shoot_wtg", "dry_root_wt", "sprout_or_transplant_to_harvest", "num_of_plants", "rep")], upper.panel = NULL)   
library(car)
vif(m2)
