...
library(corrplot)    # For nicer correlation matrices
library(tidyr)
library(ggplot2)
library(EnvStats)
library(lubridate)
...
vars <- c("tot_shoot_wtg", "florets", "d13c", "c", "d15n", "n", "p", "cn_ratio", "np_ratio", "amf_rlc", "arb_rlc", "vesicle_rlc", "am_hyphae_rlc", "dse_rlc", "lse_rlc")
...
pretty_names <- c(
  ...
  "dse_rlc" = "Dark Septate Endophytes",
  "am_hyphae_rlc" = "AM Hyphae",
  "arb_rlc" = "Arbuscules",
  "fine_endo_rlc" = "Fine Endophytes",
  "coil_rlc" = "Coils",
  "lse_rlc" = "Light Septate Endophytes",
  "florets" = "Inflorescence",
  "vesicle_rlc" = "Vesicles",
  "amf_in_dry_soil" = "AMF in Dry Soil",
  "amf_rlc" = "AMF Root Colonization Total",
...
corr_for_treatment <- function(data_subset, label, reverse_palette = FALSE, upper_triangle = TRUE) {
  corr_mat <- data_subset %>%
    select(all_of(heatmap_vars)) %>%
    cor(use = "pairwise.complete.obs")

  p_mat <- matrix(NA, nrow = length(heatmap_vars), ncol = length(heatmap_vars), dimnames = list(heatmap_vars, heatmap_vars))
  for (i in seq_along(heatmap_vars)) {
    for (j in seq_along(heatmap_vars)) {
      x <- data_subset[[heatmap_vars[i]]]
      y <- data_subset[[heatmap_vars[j]]]
      ok <- complete.cases(x, y)
      if (sum(ok) >= 4) {
        p_mat[i, j] <- tryCatch(cor.test(x[ok], y[ok], method = "pearson")$p.value, error = function(e) NA_real_)
      }
    }
  }
  ...
  corr_long$var_x <- as.character(corr_long$Var1)
  corr_long$var_y <- as.character(corr_long$Var2)
  corr_long$cor <- as.numeric(corr_long$Freq)
  corr_long <- corr_long[, c("var_x", "var_y", "cor")]
  
  # Convert p-value matrix to long format
  p_long <- as.data.frame(as.table(p_mat))
  p_long$var_x <- as.character(p_long$Var1)
  p_long$var_y <- as.character(p_long$Var2)
  p_long$p_value <- as.numeric(p_long$Freq)
  p_long <- p_long[, c("var_x", "var_y", "p_value")]
  
  corr_long <- left_join(corr_long, p_long, by = c("var_x", "var_y"))
  ...
  corr_long <- corr_long %>%
    mutate(
      ...
      label_text = paste0(sprintf("%.2f", cor), ifelse(stars != "", paste0("\n", stars), "")),
      var_x = factor(var_x, levels = heatmap_vars),
      var_y = factor(var_y, levels = heatmap_vars)
    )

  if (upper_triangle) {
    corr_long <- corr_long %>% filter(as.numeric(var_x) <= as.numeric(var_y))
  }

  palette_scale <- if (reverse_palette) {
  ...
  diag_labels <- tibble(
    var = heatmap_vars,
    var_x = factor(var, levels = heatmap_vars),
    var_y = factor(var, levels = heatmap_vars)
  )

  ggplot(corr_long, aes(x = var_x, y = var_y, fill = cor)) +
    geom_tile(color = "white") +
    geom_text(aes(label = label_text), size = 2.5, color = "black") +
    geom_text(data = diag_labels, aes(x = var_x, y = var_y, label = var, fill = NULL),
              size = 3, fontface = "bold", color = "black", hjust = 1.1, vjust = 0.1) +
    palette_scale +
    labs(title = sprintf("Variable × Variable Correlation: %s", label), x = NULL, y = NULL) +
...
p_drought_vars <- corr_for_treatment(filter(ds, treatment == "Droughted"), "Droughted", reverse_palette = FALSE, upper_triangle = TRUE)
p_watered_vars <- corr_for_treatment(filter(ds, treatment == "Watered"), "Watered", reverse_palette = TRUE, upper_triangle = TRUE)
p_all_vars <- corr_for_treatment(ds, "All Treatments", reverse_palette = FALSE, upper_triangle = TRUE)
...
p_ratio_values <- function(mat_data, dat) {
  p_mat <- matrix(NA, nrow = ncol(mat_data), ncol = ncol(mat_data), dimnames = list(colnames(mat_data), colnames(mat_data)))
  var_names <- colnames(mat_data)
...
p_ratio_mat <- p_ratio_values(ratio_by_genotype %>% select(all_of(ratio_vars)), ratio_by_genotype)

# Convert correlation matrix to long format
corr_ratio_long <- as.data.frame(as.table(corr_ratio_mat))
corr_ratio_long$var_x <- as.character(corr_ratio_long$Var1)
corr_ratio_long$var_y <- as.character(corr_ratio_long$Var2)
corr_ratio_long$cor <- as.numeric(corr_ratio_long$Freq)
corr_ratio_long <- corr_ratio_long[, c("var_x", "var_y", "cor")]

# Convert p-value matrix to long format
p_ratio_long <- as.data.frame(as.table(p_ratio_mat))
p_ratio_long$var_x <- as.character(p_ratio_long$Var1)
p_ratio_long$var_y <- as.character(p_ratio_long$Var2)
p_ratio_long$p_value <- as.numeric(p_ratio_long$Freq)
p_ratio_long <- p_ratio_long[, c("var_x", "var_y", "p_value")]

corr_ratio_long <- left_join(corr_ratio_long, p_ratio_long, by = c("var_x", "var_y"))

corr_ratio_long <- corr_ratio_long %>%
  mutate(
    stars = case_when(
      !is.na(p_value) & p_value <= 0.001 ~ "***",
      !is.na(p_value) & p_value <= 0.01 ~ "**",
    ...
    var_x = factor(gsub("^ratio_", "", var_x), levels = heatmap_vars),
    var_y = factor(gsub("^ratio_", "", var_y), levels = heatmap_vars)
  )

p_ratio_vars <- ggplot(corr_ratio_long, aes(x = var_x, y = var_y, fill = cor)) +
  geom_tile(color = "white") +
  geom_text(aes(label = stars), size = 3, color = "black", vjust = 0.5) +
  scale_fill_gradient2(low = "#2166AC", mid = "white", high = "#B2182B", midpoint = 0, limits = c(-1, 1), name = "Correlation") +
